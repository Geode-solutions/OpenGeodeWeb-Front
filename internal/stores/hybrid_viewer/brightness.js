...
function computeAverageBrightness(rect, options) {
  ...
  const {
    ...
    relW,
    relH
  } = mapRect(rect, latestImage, canvas.getBoundingClientRect());
  if (relW <= 0 || relH <= 0) {
    return BACKGROUND_GREY_VALUE / RGB_MAX;
  }
  ...
  try {
    ...
    const {
      data
    } = offscreenCtx.getImageData(0, 0, SAMPLE_SIZE, SAMPLE_SIZE);
    let minBrightness = 1;
...
function useHybridViewerBrightness() {
  ...
  const offscreenCtx = offscreenCanvas ? offscreenCanvas.getContext("2d", {
