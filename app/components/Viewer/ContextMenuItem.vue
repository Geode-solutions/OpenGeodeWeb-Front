...
<template>
  <v-sheet class="menu-item-container transition-swing" color="transparent">
    <v-tooltip :location="props.itemProps.tooltip_location" :origin="props.itemProps.tooltip_origin">
      <template v-slot:activator="{ props: tooltipProps }">
        <v-btn icon :active="is_active" @click.stop="toggleOptions" v-bind="tooltipProps"
          class="menu-btn bg-white border" elevation="2">
    ...
    <v-sheet v-if="is_active" ref="optionsRef" class="menu-options d-flex align-center pa-0" :class="optionsClass"
      :style="optionsStyle" color="transparent" @click.stop>
      <OptionCard :title="props.tooltip" width="320" :max-height="maxCardHeight">
        <slot name="options" />
      </OptionCard>
    </v-sheet>
  </v-sheet>
