...
export const useHybridViewerStore = defineStore("hybridViewer", () => {
  ...
  const status = ref(Status.NOT_CREATED),
    camera_options = reactive({}),
    genericRenderWindow = reactive({});
  const is_moving = ref(false),
    zScale = ref(1),
    latestImage = ref(undefined);
  ...
  function setCameraOrientation(orientation) {
    const config = ORIENTATIONS[orientation.toLowerCase()],
      ...
      camera = renderer.getActiveCamera(),
      startState = getCameraOptions(camera);
    ...
    const duration =
      dot(camera.getDirectionOfProjection(), config.position) > ALIGNMENT_THRESHOLD
        ? LONG_ANIMATION_DURATION
        : SHORT_ANIMATION_DURATION;
    is_moving.value = true;
  ...
  function syncRemoteCamera() {
    const camera = genericRenderWindow.value.getRenderer().getActiveCamera(),
      params = { camera_options: getCameraOptions(camera) };
  ...
  async function resize(width, height) {
    ...
    const webGLRenderWindow = genericRenderWindow.value.getApiSpecificRenderWindow(),
      canvas = webGLRenderWindow.getCanvas();
    canvas.width = width;
