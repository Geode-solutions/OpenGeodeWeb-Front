import { ACTOR_COLOR, BACKGROUND_COLOR, EASE_EXPONENT, SHORT_ANIMATION_DURATION, WHEEL_TIME_OUT_MS } from "@ogw_front/utils/vtk/constants";
import { animateCamera, applyCameraOptions, computeAverageBrightness, getCameraOptions, performCameraOrientation } from "@ogw_front/utils/hybrid_viewer";
...
export const useHybridViewerStore = defineStore("hybridViewer", () => {
  ...
  async function initHybridViewer() {
    ...
    camera.onModified(() => { Object.assign(camera_options, getCameraOptions(camera)); });
  ...
  async function addItem(id) {
    ...
    mapper.setInputData(polydata); actor.getProperty().setColor(ACTOR_COLOR); actor.setMapper(mapper);
    const renderer = genericRenderWindow.value.getRenderer();
    const isFirst = renderer.getActors().length === 0;
    renderer.addActor(actor); if (isFirst) { renderer.resetCamera(); }
  ...
  function removeItem(id) {
    ...
    genericRenderWindow.value.getRenderWindow().render(); delete hybridDb[id];
  }

  function setVisibility(id, visibility) {
    if (!hybridDb[id]) { return; }
    hybridDb[id].actor.setVisibility(visibility); genericRenderWindow.value.getRenderWindow().render();
  }

  async function setZScaling(z_scale) {
    zScale.value = z_scale; const renderer = genericRenderWindow.value.getRenderer();
    for (const actor of renderer.getActors()) { if (actor !== gridActor) { const scale = actor.getScale(); actor.setScale(scale[0], scale[1], z_scale); } }
    renderer.resetCamera(); genericRenderWindow.value.getRenderWindow().render();
    const schema = viewer_schemas?.opengeodeweb_viewer?.viewer?.set_z_scaling;
    if (schema) { await viewerStore.request(schema, { z_scale }); }
    remoteRender();
  }

  function resetCamera() { genericRenderWindow.value.getRenderer().resetCamera(); genericRenderWindow.value.getRenderWindow().render(); syncRemoteCamera(); }

  function setCamera(new_camera_options) {
    const camera = genericRenderWindow.value.getRenderer().getActiveCamera();
    const startState = getCameraOptions(camera);
    is_moving.value = true; if (imageStyle) { imageStyle.opacity = 0; }
    animateCamera({
      camera, startState, targetState: new_camera_options, duration: SHORT_ANIMATION_DURATION, bumpMultiplier: 0, easeExponent: EASE_EXPONENT,
      onUpdate: () => genericRenderWindow.value.getRenderWindow().render(),
      onEnd: () => { applyCameraOptions(camera, new_camera_options); genericRenderWindow.value.getRenderWindow().render(); is_moving.value = false; syncRemoteCamera(); },
    });
  }

  function setCameraOrientation(orientation) {
    performCameraOrientation({
      orientation, camera: genericRenderWindow.value.getRenderer().getActiveCamera(), renderer: genericRenderWindow.value.getRenderer(),
      renderWindow: genericRenderWindow.value.getRenderWindow(),
      onStart: () => { is_moving.value = true; if (imageStyle) { imageStyle.opacity = 0; } },
  ...
  function syncRemoteCamera() {
    ...
    viewerStore.request(viewer_schemas.opengeodeweb_viewer.viewer.update_camera, { camera_options: options }, {
      response_function: () => { remoteRender(); Object.assign(camera_options, options); },
    });
  }
  ...
  function setContainer(container) {
    ...
    const webGLRenderWindow = genericRenderWindow.value.getApiSpecificRenderWindow();
    webGLRenderWindow.setUseBackgroundImage(true); imageStyle = webGLRenderWindow.getReferenceByName("bgImage").style;
    Object.assign(imageStyle, { transition: "opacity 0.1s ease-in", zIndex: 1 });
    resize(container.value.$el.offsetWidth, container.value.$el.offsetHeight);
    useMousePressed({
      target: container,
      onPressed: (event) => { if (event.button === 0) { is_moving.value = true; event.stopPropagation(); if (imageStyle) { imageStyle.opacity = 0; } } },
  ...
  async function resize(width, height) {
    ...
    canvas.width = width; canvas.height = height; await nextTick();
    webGLRenderWindow.setSize(width, height); viewStream.setSize(width, height);
    genericRenderWindow.value.getRenderWindow().render(); remoteRender();
  }

  function getAverageBrightness(rect) { return computeAverageBrightness(rect, { latestImage: latestImage.value, offscreenCtx, offscreenCanvas, genericRenderWindow: genericRenderWindow.value }); }
  ...
  async function importStores(snapshot) {
    ...
    if (snapshot.camera_options) {
      ...
      genericRenderWindow.value.getRenderWindow().render(); syncRemoteCamera();
    }
  }

  function clear() {
    const renderer = genericRenderWindow.value.getRenderer();
    for (const actor of renderer.getActors()) { renderer.removeActor(actor); }
    genericRenderWindow.value.getRenderWindow().render(); for (const id of Object.keys(hybridDb)) { delete hybridDb[id]; }
  }
