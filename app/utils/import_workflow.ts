...
async function importItem(item: NewDataItem): Promise<string> {
  ...
  const registerTask = addDataTask.then(async () => {
    ...
    return dataStore.registerObject(item.id, item.name ?? item.id);
  });
...
async function importFile(filename: string, geode_object_type: string): Promise<string> {
...
async function importWorkflow(files: FileToImport[]): Promise<string[]> {
  ...
  async function processChunk(chunkIndex: number): Promise<void> {
    ...
    const chunk_results = await Promise.all(
      chunk.map(
        async ({ filename, geode_object_type }) => await importFile(filename, geode_object_type),
      ),
    );
